<template>
  <div class="app-container">
    ...
    <el-table :data="list"
      ...
      <el-table-column align="center" label="序" width="60">
        <template slot-scope="scope">
          <span v-text="getIndex(scope.$index)"> </span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="岗位ID" prop="roleId" width="100"></el-table-column>
      <el-table-column align="center" label="岗位名称" prop="roleName" width="150"></el-table-column>
      <el-table-column align="center" label="用户" width="auto">
        <template slot-scope="scope">
          <span>{{ getUserList(scope.row.users) }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="菜单&权限" width="420">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.roleName===adminName"
                  type="success">全部
          </el-tag>
          <div v-else>
            <div v-for="menu in scope.row.menus" style="text-align: left">
              ...
              <el-tag v-for="perm in menu.permissions"
...
<script>
import {default as myApi} from '@/utils/request'
...
export default {
  ...
  methods: {
    getUserList(userData) {
      ...
      for (let i = 0; i < 10; i++) {
        if (userData[i] != null) {
          userList += userData[i]["nickname"] + ",";
        }
      }
      userList += "... " + userData.length + " 人";
      return userList;
    },
    getAllPermisson() {
      //查询所有权限
